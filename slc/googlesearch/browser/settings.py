...
from slc.googlesearch.interfaces import IStoredCSESchema
from slc.googlesearch.interfaces import ILinkedCSESchema
from slc.googlesearch.interfaces import IStoredCSETuple
from slc.googlesearch.interfaces import ILinkedCSETuple
from slc.googlesearch.interfaces import IAdditionalParametersTuple
from slc.googlesearch.interfaces import IAdditionalParametersSchema
...
class GoogleSearchSettings(Persistent):
    ...
    # keep a "backup" copy
    # dirty hack, since getSite can return an
    # Products.Five.metaclass.FormlibValidation instance, which cannot be used
    # for IAnnotation...
    stored_list = list()
    linked_list = list()
    additionals_list = list()
    ...
    def stored_settings():
        def get(self):
            return [StoredCSETuple(l, t)
                    for (l, t) in self.settings.stored_list]
    ...
    def linked_settings():
        def get(self):
            return [LinkedCSETuple(l, t)
                    for (l, t) in self.settings.linked_list]
    ...
    def additional_settings():
        def get(self):
            return [AdditionalParametersTuple(q,)
                    for (q,) in self.settings.additionals_list]
...
additional_parameters_widget = CustomWidgetFactory(ObjectWidget,
                                                   AdditionalParametersTuple)
