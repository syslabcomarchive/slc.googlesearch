...
class ICSEPortlet(IPortletDataProvider):
    selected_CSE = schema.Choice(
        title=_(u'Selected CSE'),
        description=_(u'Pick the CSE you want to use for this search box'),
        vocabulary="slc.googlesearch.vocabularies.AvailableCSE")

    selected_additionals = schema.Choice(
        title=u'Additional query parameters',
        description=_(u'Pick additional parameters that will be appended to '
                      'the query (optional)'),
...
class AddForm(base.AddForm):
    ...
    def create(self, data):
        return Assignment(
            selected_CSE=data.get('selected_CSE', ''),
            selected_additionals=data.get('selected_additionals', ''))
